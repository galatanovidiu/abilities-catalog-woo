...
final class Registry {
	 ...
	 *
	 * @var array<string,\GalatanOvidiu\AbilitiesCatalogWoo\Contracts\Ability>
	 */
	 ...
	 *
	 * @var array<int,\GalatanOvidiu\AbilitiesCatalogWoo\Contracts\CategoryProvider>
	 */
	 ...
	 *
	 * Each ability group owns a {@see CategoryProvider} (e.g. the Woo group's
	 * `Abilities\Woo\CategoryCatalog`); this merges their `categories()` and registers
			...
			if ( ! $is_readonly && ! $has_annotated_safety ) {
				_doing_it_wrong(
					...
					sprintf(
						...
						esc_html__( 'Ability "%s" was refused: it must be read-only or a write that explicitly sets annotations.destructive to a boolean. A write that omits the destructive annotation is treated as unsafe and not registered.', 'abilities-catalog-woo' ),
						esc_html( $name )
					),
					'0.1.0'
				);
	 ...
	 * load.
	 *
	 * @param \GalatanOvidiu\AbilitiesCatalogWoo\Contracts\Ability $ability The discovered ability instance.

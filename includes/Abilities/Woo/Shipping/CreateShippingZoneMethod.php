...
/**
 * Write ability: `wc-shipping/create-shipping-zone-method`.
 *
 ...
 *
 * To change an existing instance use `wc-shipping/update-shipping-zone-method`; the
...
final class CreateShippingZoneMethod implements ConditionalAbility {
	...
	public function name(): string {
		return 'wc-shipping/create-shipping-zone-method';
	}
	...
	public function args(): array {
		return array(
			...
			'description'         => __( 'Adds a shipping method instance to a shipping zone and returns the created instance: its instance_id, method_id, title, enabled, order, and a settings_summary. Provide zone_id (discover with wc-shipping/list-shipping-zones) and method_id, the method TYPE to add — e.g. flat_rate, free_shipping, or local_pickup (discover available types with wc-shipping/list-shipping-methods). The optional settings object configures the method and is keyed by that method type\'s own setting ids (e.g. {"cost": "10", "title": "Standard"} for flat_rate); its keys depend on the method type, and the route applies only the keys it recognizes and ignores unknown ones. After creating, see the configured setting ids with wc-shipping/get-shipping-zone-method. To change an existing instance use wc-shipping/update-shipping-zone-method; the method type is fixed at creation. An unknown zone_id is rejected by the route with woocommerce_rest_shipping_zone_invalid 404.', 'abilities-catalog-woo' ),
			...
			'input_schema'        => array(
				...
				'properties'           => array(
					'zone_id'   => array(
						'type'        => 'integer',
						'minimum'     => 1,
						'description' => __( 'The shipping zone to add the method to. Discover zone ids with wc-shipping/list-shipping-zones.', 'abilities-catalog-woo' ),
					),
					'method_id' => array(
						'type'        => 'string',
						'description' => __( 'The shipping method TYPE to add, e.g. flat_rate, free_shipping, or local_pickup (not an instance id). Discover the available types with wc-shipping/list-shipping-methods.', 'abilities-catalog-woo' ),
					),
					'settings'  => array(
						'type'                 => 'object',
						'description'          => __( 'Optional configuration for the method, keyed by the method type\'s own setting ids (e.g. {"cost": "10", "title": "Standard"} for flat_rate). The valid keys depend on the method type; the route applies only the keys it recognizes for the chosen type and ignores unknown ones. Discover a configured method\'s setting ids with wc-shipping/get-shipping-zone-method.', 'abilities-catalog-woo' ),

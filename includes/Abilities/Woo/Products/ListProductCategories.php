 ...
 * {@see ProductTermListShaper::termSummary()}, so a consumer scans the category
 * tree without the raw category body (which carries `display`, `image`, and
 * `menu_order`). Exposes a minimal, useful subset of the terms controller's
 ...
 * @since 0.1.0
 */
final class ListProductCategories implements ConditionalAbility {
		...
		return array(
			...
			'input_schema'        => array(
				...
				'properties'           => array(
					'search'     => array(
						'type'        => 'string',
						'description' => __( 'Limit results to categories whose name matches a search term.', 'abilities-catalog-woo' ),
					),
					'per_page'   => array(
						'type'        => 'integer',
						'minimum'     => 1,
						'maximum'     => 100,
						'default'     => 100,
						'description' => __( 'Maximum number of categories to return (1-100). Defaults to 100, which covers every category on a typical store.', 'abilities-catalog-woo' ),
					),
					'page'       => array(
						'type'        => 'integer',
						'minimum'     => 1,
					...
					'parent'     => array(
						'type'        => 'integer',
						'minimum'     => 0,
						'description' => __( 'Limit results to categories whose parent is this category term ID. Use 0 to return only top-level categories.', 'abilities-catalog-woo' ),
					),
					'hide_empty' => array(
						'type'        => 'boolean',
						'description' => __( 'When true, exclude categories that have no published products assigned.', 'abilities-catalog-woo' ),
					),
					'orderby'    => array(
						'type'        => 'string',
						'enum'        => array( 'id', 'name', 'slug', 'count', 'term_group' ),
						'default'     => 'name',
						'description' => __( 'Sort the result set by this attribute: "name" (the default), "id", "slug", "count" (number of products), or "term_group".', 'abilities-catalog-woo' ),
					),
					'order'      => array(
						'type'        => 'string',
						'enum'        => array( 'asc', 'desc' ),
			...
			'output_schema'       => array(
				...
				'properties'           => array(
					...
					'total' => array(
						'type'        => 'integer',
						'description' => __( 'The total number of categories matching the query across all pages, read from the X-WP-Total response header. May exceed the number of returned rows when paging.', 'abilities-catalog-woo' ),
					),
				),
				'additionalProperties' => false,
			),
	 ...
	 * `manage_terms` cap of the `product_cat` taxonomy, registered as
	 * `manage_product_terms`). This is a coarse, object-independent guard; the
	 * wrapped route applies any per-term checks. The explicit activity guard keeps
	 * the denial clean when WooCommerce is inactive and the capability is unmapped.
	 *
	 * @param mixed $input The validated input data.
	 * @return bool True if the current user may read product categories.
	 */
	 ...
	 *
	 * @param mixed $input The validated input data.
	 * @return array<string,mixed>|\WP_Error The list of categories, or the REST error.
	 */

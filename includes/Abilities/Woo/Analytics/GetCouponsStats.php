 ...
 * discount given), `coupons_count`, and `orders_count` — plus `intervals_count`
 ...
 * @since 0.1.0
 */
final class GetCouponsStats implements ConditionalAbility {
			...
			'input_schema'        => array(
				...
				'properties'           => array(
					...
					'per_page' => array(
						...
						'maximum'     => 100,
						'default'     => 10,
						'description' => __( 'Maximum number of per-interval buckets the report computes (1-100). Defaults to 10. Affects how many buckets back intervals_count, not the returned totals.', 'abilities-catalog-woo' ),
					),
					'page'     => array(
						'type'        => 'integer',
						'minimum'     => 1,
						'default'     => 1,
						'description' => __( 'Page of per-interval buckets to compute, for paging past the first per_page buckets. Defaults to 1.', 'abilities-catalog-woo' ),
					),
				),
	 ...
	 *
	 * The wrapped `/wc-analytics/reports/coupons/stats` route resolves its
	...
	public function execute( $input ) {
		...
		$request = new WP_REST_Request( 'GET', '/wc-analytics/reports/coupons/stats' );
	...
	/**
	 * The closed `totals` object schema for the coupons-stats KPIs.
	 *
	 * Field order matches the projection in {@see execute()} (floats first, then
	 * ints). Every field is `readonly` in the wrapped controller
	 * (`Coupons/Stats/Controller.php::get_item_properties_schema()`), so all are
	...
	private function totalsSchema(): array {
		return array(
			...
			'properties'           => array(
				'amount'        => array(
					'type'        => 'number',
					'description' => __( 'Net discount amount given by coupons across the range, in the store currency.', 'abilities-catalog-woo' ),
				),
				'coupons_count' => array(
					'type'        => 'integer',
				...
				'orders_count'  => array(
					'type'        => 'integer',
					'description' => __( 'Number of discounted orders (orders that used at least one coupon) in the range.', 'abilities-catalog-woo' ),
				),
			),
			'additionalProperties' => false,
		);

 ...
 */
final class GetProductReviewTest extends TestCase {

	/**
	 * The closed key set the ability returns for one review row.
	 *
	 * @var list<string>
	 */
	...
	public function test_admin_reads_review_detail(): void {
		...
		$this->assertSame( 'ada@example.com', $result['reviewer_email'] );
		$this->assertSame( 5, $result['rating'] );
		$this->assertSame( 'Great product, works well.', $result['review'] );
	...
	public function test_subscriber_is_denied(): void {
		$product_id = $this->seedProduct( 'Seeded Product' );
		$review_id  = $this->seedReview( $product_id, 'Ada Reviewer', 'ada@example.com', 'Great.', 5 );
		...
		$result = $ability->execute( array( 'id' => $review_id ) );
